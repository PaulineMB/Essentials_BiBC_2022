...
Put simply, for Runge-Kutta methods, there are different "orders" i.e. using a different number of the in class described k_i values to estimate the slope of the function in a different number of points and with different info. By combining 2 Runge-Kutta order methods, here RK2 and RK3, and comparing their results one gets an idea of the error: RK3 will be more precise than RK2 and this will matter more if precision is being an issue. The size of this error is then used to decide on the size of the integration timestep at the next stage.
